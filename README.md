...
Тесты запускаются через Vitest, а отчёт о покрытии публикуется в Codecov.
...
### 📲 Сборка APK/IPA через Capacitor

Чтобы получить полноценный установщик, используйте [Capacitor](https://capacitorjs.com/):

1. Выполните `npm install`, чтобы установить зависимости, включая `@capacitor/*`.
2. Инициализируйте проект командой `npx cap init` (в репозитории уже есть файл `capacitor.config.ts` с `webDir: dist`).
3. Добавьте платформу: `npx cap add android` или `npx cap add ios`.
4. Соберите веб‑версию `npm run build`.
5. Скопируйте файлы в нативный проект `npx cap sync`.
6. Запустите `npx cap open android` (или `ios`) и соберите APK/IPA в Android Studio или Xcode.
7. Установите приложение на устройство обычным способом через подключенную IDE.
...
- Создайте компонент в `src/pages/`
...
4. Скопируйте идентификатор клиента и API key в файл `.env` по образцу `.env.example`.
   Переменные из этого файла будут автоматически подхвачены сборкой через `dotenv-webpack`.

### Создание плагина

1. Поместите HTML или JS файл в папку `plugins`.
2. В скрипте используйте `window.parent.postMessage({ type: 'addButton', id: 'my', label: 'My Plugin' }, '*')`,
   чтобы добавить кнопку в интерфейс.
3. Отправьте `postMessage({ type: 'addCommand', id: 'hello', title: 'Hello' }, '*')`,
   чтобы объявить пользовательскую команду.
4. Ваш плагин может реагировать на события `message` и обмениваться данными с приложением.

## 📄 Лицензия

MIT License - используйте свободно для личных и коммерческих проектов.

🤝 Вклад в проект приветствуется! Ознакомьтесь с [CONTRIBUTING.md](CONTRIBUTING.md) перед отправкой изменений.
Перед коммитом выполняйте `npm run lint && npm run format:check && npm test`.
